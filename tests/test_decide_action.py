...
def test_heuristic_skip_below_threshold():
    assert decide_action(_ctx(0.79), streak=8, saver=False)[0] == "skip"


def test_heuristic_double_when_p_both_clears_threshold():
    # streak 8 -> threshold 0.55; 0.80*0.78=0.624 >= 0.55 -> double
    assert decide_action(_ctx(0.80, second_p=0.78), streak=8, saver=False)[0] == "double"


def test_heuristic_single_when_p_both_below_threshold():
    # streak 8 -> 0.55; 0.80*0.60=0.48 < 0.55 -> single
    assert decide_action(_ctx(0.80, second_p=0.60), streak=8, saver=False)[0] == "single"


def test_no_diff_game_forces_single():
    assert decide_action(_ctx(0.90, has_diff=False, second_p=None), streak=8, saver=False)[0] == "single"


def test_allow_double_false_forces_single():
    assert decide_action(_ctx(0.90, second_p=0.90, allow_double=False), streak=8, saver=False)[0] == "single"


def test_sprint_streak_never_doubles():
    # streak 56 -> _double_threshold None -> single even with great candidates
    assert decide_action(_ctx(0.95, second_p=0.95), streak=56, saver=False)[0] == "single"
...
def test_mdp_double_passes_through_when_executable(monkeypatch):
    ...
    assert decide_action(_ctx(0.50, second_p=0.50, mdp={"x": 1}), streak=12, saver=True)[0] == "double"
...
def test_mdp_skip_overrides_strong_candidates(monkeypatch):
    ...
    assert decide_action(_ctx(0.95, second_p=0.95, mdp={"x": 1}), streak=12, saver=True)[0] == "skip"
...
def test_mdp_double_downgraded_when_no_diff_game(monkeypatch):
    ...
    assert decide_action(_ctx(0.50, has_diff=False, second_p=None, mdp={"x": 1}), streak=12, saver=True)[0] == "single"
...
def test_mdp_double_downgraded_when_allow_double_false(monkeypatch):
    ...
    assert decide_action(_ctx(0.50, second_p=0.50, allow_double=False, mdp={"x": 1}), streak=12, saver=True)[0] == "single"
...
def test_select_pick_double_matches_decide_action(tmp_path, monkeypatch):
    ...
    sp_action = "double" if result.pick_result.daily.double_down is not None else "single"
    direct = decide_action(_ctx(0.85, second_p=0.80), streak=8, saver=False)[0]
...
def test_select_pick_single_game_matches_decide_action(tmp_path, monkeypatch):
    ...
    sp_action = "double" if result.pick_result.daily.double_down is not None else "single"
    direct = decide_action(_ctx(0.90, has_diff=False, second_p=None), streak=8, saver=False)[0]
