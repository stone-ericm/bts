...
- **Shadow model** (v2 since 2026-07-08): `CONTEXT_COLS` (ump_hr, wind, hardness, indoor, park_drag_delta) run alongside production via `feature_cols_override`. Picks saved to `{date}.shadow.json`. Monitor with `bts shadow-status`; result reconciliation via `bts check-results` now writes `data/validation/context_stack_shadow_status.json` by default when shadow files exist. Use `bts shadow-backfill-results` for reviewed DD-aware recompute/audit manifests before any promotion discussion.
