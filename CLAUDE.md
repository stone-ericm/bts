...
- **Skip-policy shadow** (`skip_policy_shadow.py`, distinct from the shadow MODEL above): a counterfactual "pick-the-band" shadow POLICY testing whether the deployed skip-at-streak≥8/sub-0.796 rule costs streaks on the production scale. **Ground truth via a marker:** `select_pick` writes `data/picks/<date>/skip_decision.json` (the executable declined candidate) at a genuine MDP skip, but ONLY when `persist_skip_decision=True` — which only the scheduler's production `run_and_pick` passes (NOT preview/manual `bts run`/shadow-model). Best-effort, never affects the pick. The shadow reads markers → `{date}.policy_shadow.json`, reconciles the candidate's outcome, writes `data/validation/skip_policy_shadow_status.json` with a verdict vs the ~0.744 breakeven (`below_breakeven`=skip validated, `above_breakeven`=skip costs streaks, else accumulating). Marker fires only for a live (scheduler-only `persist_skip_decision`), **MDP-backed** (`ctx.mdp` truthy) skip. **Final-decision by delivery**: marker + `<date>.json` are both provisional (preview pre-writes tomorrow's; fallback re-delivers a cached pick), so the authoritative signal is `picks.pick_was_delivered`; a record whose date later resolves to a delivered pick is pruned. (Pre-existing related bug — `check-results` scores undelivered picks on skip days — deferred to a separate decision-record fix; see the audit doc.) CLI `bts skip-policy-shadow-update` (nightly cron 23:30 ET) / `bts skip-policy-shadow-status`; dashboard panel. Why a marker not read-only reconstruction (4 review rounds): `docs/audit/2026-06-20-skip-policy-shadow.md`.
