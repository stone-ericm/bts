...
| X-05 | **DD tripwire — interim look** (`docs/audit/2026-08-09-dd-tripwire-recompute.md`) | 7/12 measurement recipe (per-slot grading, legacy-single fallback) | DD legs 3/29→8/09 (n=57), primaries n=83 | 7/13 pre-registration (P-01) + 8/09 binding commitment | exploratory only; trigger NOT evaluated | **P-01 formal read DONE 2026-09-22** (`docs/audit/2026-09-22-dd-tripwire-formal-read.md`): first n≥80 snapshot = 2026-09-08 (reconstructed), 56/80 = .700 vs .743 stated, −4.3 pp → **trigger NOT fired**; season-end update thru 9/13 59/85 −4.9 pp (labelled). Anchors reproduce. Next look: none under P-01; calibration questions → W1.2/W1.3 |
...
| X-11 | **Decision-weighted `decision_weighted_lgbm_v0` live-forward stream** | candidate frozen `5004b1c8` (5/10) | pre-outcome slates 5/09→9/18 captured (68/79 decision days; 11 skip days missing); resolve step joins outcomes nightly — **no paired read has been taken** | P-04 | **INSUFFICIENT (inventoried 9/22):** 114 pre-outcome dirs 5/09→9/18, 114 resolved complete + verification ok, 113 with a production-pick snapshot (5/09 predates the parity rule) → eligible ≤ 113 < 120; the stream is frozen at 9/18 (no production picks after the 9/19 tail stop), so 120 is unreachable this season. 255 `stale_pick_snapshot.*` recapture side-dirs exist (provisional-pick drift) | no paired `p_57_mdp` read this season; the frozen candidate carries to 2027 forward logging or is retired in W3/W4 |
...
| X-14 | **9/22 season-board capture, first descriptive read** (`final_grab_20260922`) | none (descriptive; whole-field board) | season-best distribution of the 72,600 listed users (floor ≥4): max 39, ≥30: 49, ≥20: 2,156, median 9, 99.9th pct 28; Eric rank 2,988 with 18 (2,987 users above; 4,532 at ≥18); 67,548 distinct usernames for 72,600 ids | plan W2.1 (final-leader case series + whole-field distribution) | descriptive; not a candidate test | W2.1 memo after the 9/28 supplemental; no model/policy candidate may be motivated by these numbers without 2027 forward validation (D3) |
