...
If you must stop the run (disk, time, doubt): Ctrl-C once. The ledger already holds every sent request's intent/outcome; `status.json` will show `terminal_state` other than complete with the in-flight request marked aborted on the next reconciliation. Keep everything under the run root as evidence; do not delete, do not rerun without a fresh owner decision.
