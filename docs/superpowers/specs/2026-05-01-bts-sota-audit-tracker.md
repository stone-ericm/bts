...
- **Status (2026-05-02 v2 evening)**: v2 SHIPPED via Issue #7 — closes v1's two methodology gaps. Per-rank-1-bin `rho_pair_per_bin` correction (5-element vector) and within-fold dependence-parameter estimation (rho_PA, tau, rho_pair refit per LOSO fold's 4 training seasons). New diagnostic 5×5 lower-triangular heatmap via `pair_residual_correlation_per_cell`. v2 verdict: `corrected_pipeline_p57 = 0.0333 [0.0000, 0.1167]` → `HEADLINE_REDUCED`. **However (per Codex round 1 review of memo)**: v2 point estimate is *still below* half-headline (0.0408); the gate-class transition from v1's `BROKEN` to v2's `REDUCED` is CI-driven, not point-estimate driven. Q4 sign reversed (v1 antagonism → v2 cooperative in 2/5 folds, near-zero in 3/5) — heterogeneous, not "artifact." v2.5 priority ordered: (1) attribution ablations to isolate which methodology change drove the verdict shift, (2) v2 in-sample recompute (NOT done in v1→v2 round), (3) same-evaluator policy comparisons, (4) block-bootstrap CI replacing 5-fold percentile. See `docs/sota_audit/2026-05-02-harness-v2-comparison.md` and `data/validation/falsification_harness_v2_2026-05-03.json`.
