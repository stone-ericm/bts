...
- **Status update 2026-05-08**: Reactivated as the next concrete candidate-cycle direction after pooled-policy closeout. Pre-registration draft: `docs/sota_audit/2026-05-08-fresh-audit-pre-registration.md`. The draft names `decision_weighted_lgbm_v0` as a sensitivity-weighted LightGBM candidate but intentionally leaves launch blocked until exact code and tests freeze the weighting function and comparison path.
- **Status**: pre-registered direction / candidate implementation unfrozen.
- **Next action**: Implement the smallest sensitivity-weighted LightGBM candidate behind a non-production experiment path. Freeze the sample-weight function, deterministic seed settings, artifact schema, and comparison script in a PR before any cloud smoke or live-forward fresh-target evaluation.
