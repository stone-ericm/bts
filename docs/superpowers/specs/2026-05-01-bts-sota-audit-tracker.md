...
- **Status (2026-05-02 v2 evening)**: v2 SHIPPED via Issue #7 — closes v1's two methodology gaps. Per-rank-1-bin `rho_pair_per_bin` correction (5-element vector) and within-fold dependence-parameter estimation (rho_PA, tau, rho_pair refit per LOSO fold's 4 training seasons). New diagnostic 5×5 lower-triangular heatmap via `pair_residual_correlation_per_cell`. v2 verdict: `corrected_pipeline_p57 = 0.0333 [0.0000, 0.1167]` → **`HEADLINE_REDUCED`** (gate-class transition from v1's `HEADLINE_BROKEN`). v1's apparent Q4 antagonism did NOT survive fold-local re-estimation — it was a finite-sample artifact. See `docs/sota_audit/2026-05-02-harness-v2-comparison.md` and `data/validation/falsification_harness_v2_2026-05-03.json`. Open follow-up: fold-bootstrap CI methodology (current 5-fold percentile is essentially min/max).
