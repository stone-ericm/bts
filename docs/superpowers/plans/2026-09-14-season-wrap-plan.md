...
1. **Reviewed one-shot runbook or thin wrapper** (Codex code review before it runs) specifying: tab names to capture; allocation order (D5 default: final-leader ranks across the named tabs; early-cohort reserve only if D5 changes); exact distinct-user cap (300 by `user_id`, not username); page/request ceilings per endpoint; **deep paging OFF** unless explicitly included in the bound; raw HTTP response retention (gzipped bodies + timestamps + status) *before* parsing; per-request success/partial/abort status file.
...
### W0.8 Research-capture continuation (only if D8 = continue) **[r2]**
Continuation is a **separate forecast/research capture stream** that preserves the official tail decision, contest state, delivery settings and every original protocol's eligibility rules. It does not override the tail stop, does not touch `decision.json` semantics, and adds **no** eligible skip-policy observations (`record_skip_from_decision` requires `is_reach57_mdp_skip`; tail-objective skips are excluded by construction). If a private counterfactual *policy* trajectory is wanted, it gets a frozen rule, independent state and an explicit synthetic-trajectory label. Tests: restart behavior; zero contest/delivery side effects. What it buys: more diagnostic forecast observations for W1.2/W2.3, not extra shadow trials.
...
| D3 | **How should remaining unseen 2026 data be used?** (early) | Reserve eligible unseen outcomes for existing frozen tests; **or** spend them on exploration and validate new ideas prospectively in 2027. Existing protocol history cannot be waived | exploration spends the data's independent-test value |
...
| D5 | **Which profiles use the existing 300-profile cap?** (early; default recorded) | Keep the authorized final-leader allocation (Eric, 9/14); **or** reserve an exact number for the early cohort — better follow-up, fewer final leaders | weaker field comparison either way; neither identifies the whole field |
