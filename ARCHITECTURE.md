...
records (missing `action`/`scoreable`/`date`, or wrong `schema_version`) as missing.

**Warmup / silent-pass hardening (2026-08-14, after the 2026-08-13 undelivered-pick incident;
commits `1b50b78`+`224ddce`).** MLB's "Warmup" (statusCode PW) is the ONLY abstract-Live
pre-first-pitch state; `classify_pick_lock_state` previously read it as `game_started_or_final`,
which classification-locked an undelivered candidate and passed the day with zero alerts (E3 was
gated on `state.pick_locked` — set by that very lock). Now: Warmup does not lock an existing
undelivered pick (matched on the `code` field newly returned by `get_game_statuses_detailed`,
detailed-string fallback), but the fresh-candidate pool still EXCLUDES warmup games (entry closes
first pitch − 5 min; split predicates, `pick_candidate_status_is_available`); an undelivered pick
with `delivery_attempted=True` locks on that durable marker BEFORE any status lookup (reselection
would otherwise erase the crash-idempotency guard → duplicate send) and a classification-lock on it
finalizes as a scoreable `locked_unconfirmed` commit at the run_day chokepoint; the chokepoint never
clobbers an existing scoreable decision (resyncs the flag instead); `committed_pick_written` is set
only when `write_decision` actually returned a record; E3 gates on `committed_pick_written` plus the
on-disk decision record (not `pick_locked`); the pre-cascade short-circuit locks on any scoreable
decision (committed = immutable across restarts); and `select_pick` with an empty fresh pool returns
a still-deliverable current pick UNLOCKED so the fallback path can deliver it. This also closed the
two previously-accepted private-mode edges: a private commit whose decision-write fails now fires E3
(alert-on-ambiguity; still unscored), and a private pick IS re-locked on restart via the scoreable
record. Remaining backlog (docs/optimization-ideas.md §Scheduler Edge Cases): an independent
cron-side day-outcome watchdog; the no-games-day early return bypasses step 5b + EOD health.
...
- **Submission cutoff + deadline model (2026-08-30, `docs/audit/2026-08-30-late-pick-delivery.md`):** the BTS cutoff is first pitch − 5 min of the EARLIEST committed slot — one constant, `picks.SUBMISSION_CUTOFF_MIN` (`submission_cutoff_et(daily)`). **Hard guard:** `_deliver_and_lock_pick` refuses at/after the cutoff in every mode (archives `refused_delivery_*.json`, removes `<date>.json`, records `state.delivery_refusals`, DMs a CRITICAL, leaves the day unlocked so later cycles re-pick from enterable games — `_games_past_cutoff` → `select_pick(unavailable_game_pks=…)`, exclusion only, never a margin). `DailyPick.delivered_at` is the send time (`run_time` is selection time). **`fallback_deadline_min` is the latest cascade START** (the refresh it launches takes a full cascade); it is floored at `5 + cascade_budget_min + operator_reserve_min` (defaults 12 + 10 → 27; box value 35 unchanged). **Deferral is planned AFTER the refresh** (`plan_fallback_action`) against the live clock and re-synced confirmations: a gap-rule block defers only if the CONTENDER's confirmation window can finish (`max(start, now) + budget ≤ cutoff − reserve`), else it DELIVERS the enterable pick (policy change 2026-08-30 — the old any-pending-window rule abandoned Perdomo 8/27, an Arraez DD 8/29 and Kwan 8/30); a projected PRIMARY keeps the 2026-07-06 abandon-for-the-later-slate choice. Overrun scheduled checks the refresh already covered are coalesced. Cascade durations are measured (`runs_completed[*].duration_sec`, `state.fallback_refreshes`) and raise the budget intraday (`effective_cascade_budget_min`: slower of the last two + 2). Latency: `pull_feeds` no longer sleeps between cached feeds and `_refresh_season_data` runs once per (season, yesterday) (`data/processed/.refreshed_*`; `BTS_REFRESH_ALWAYS=1` forces) — cascades expected ~5 min instead of ~15.5 (verify post-deploy via `runs_completed[*].duration_sec`).
...
- `simulate/tail_policy.py` — the late-season objective switch (2026-09-03). `mdp_objective(streak, days)` picks `reach57` vs `emax_season_best` from state alone; the tail table `[streak, best, days<=28, saver, bin]` is the exact expected-season-best DP (port of the validated audit solver) with an explicit stop rule (skip iff `min(57, s+2d) <= best`) and play-first tie-breaking. Separate artifact `data/models/mdp_tail_policy.npz`, strict loader bound to the base policy's sha256; `strategy.resolve_policy_decision` produces ONE `PolicyDecision` (objective, best trust, effective best, tail sha, degraded reason) reused by logs, skip DM/dashboard, decision.json v3 and the skip-shadow/census filters. `contest_state.classify_best_streak` is the trust boundary for the contest best. See `docs/audit/2026-09-03-emax-tail-policy.md`.
...
| `fallback_defer` | 1 | fallback-defer events: makes the archive-unsafe-fallback-candidate path visible; pages only if a defer breaks the never-miss guarantee. `same_pick` compares BOTH slots (2026-08-30). |
| `late_delivery` | 1 | (added 2026-08-30) EOD backstop for the delivery-cutoff guard: the day's pick sent at/after first pitch − 5 (`DailyPick.delivered_at`, fallback `scheduler_state.pick_locked_at`) → CRITICAL; a recorded `delivery_refusals` entry → CRITICAL; a send inside the operator reserve (10 min) → WARN. `fallback_defer`'s "a delivered file exists" is exactly how the 13:36 DM for a 13:35 cutoff read as healthy. |
...
### Live-forward candidate capture + research-only forecast stream (D8, 2026-09-14)
`scripts/live_forward_capture_once.py` (user timer `bts-live-forward-capture.timer`, every 15 min 05:00–22:00 ET; resolve timer 4×/day) exports the frozen decision-weighted candidate's and production's top-10 ranked slates for the day into `data/validation/decision_weighted_lgbm_v0_live_forward/<date>/`, verifying parity with the locked production pick (`--require-production-pick-snapshot`). It idles at `pending_pick` when no pick file exists — which is every MDP/tail SKIP day (11 of 79 decision days 5/09→9/18 had no slate). **`--capture-research-on-skip`** (unit flag since `f1b07e9`): when the scheduler has persisted a CURRENT skip intent (`scheduler_state.json.final_skip_candidate`; `skip_notified_at` alone is history) and `decision.json` is not scoreable, the slates are exported into a SEPARATE root `…_live_forward_research/<date>/` (never eligible for the official #16 read, `discover_dates` never sees it), verified without the pick-snapshot requirement, and published in two phases: sidecar `research_capture.json` (identity, provenance, timestamps, sha256 of every contract file) then `research_capture.accepted.json` whose `published_at` is sampled AFTER the sidecar rename. Guards: root-overlap refusal, no symlinked date dirs, start before first pitch − 5 min and publish before first pitch (else `.late.*`), fail closed without game times, partial/corrupt/unaccepted dirs → `.partial.*` and redone only pregame; all reads side-effect-free (never `load_state`, which quarantines corrupt files). Verified live 9/19–9/21 after the tail stop.
...
**2026-09-22 hardening (season wrap, Codex-reviewed):**
- **C-01 parser fix.** `parse_leaderboard_response` used `activeStreak` for EVERY tab, so historical `leaderboard_snapshots` rows for `all_season` / `all_time` (2026-05-01 → 07-04) hold the ACTIVE streak, not the tab's ranking quantity. Now `streak` is tab-semantic (season best on `all_season`, all-time best on `all_time`, active on `active_streak`/`yesterday`) and two explicit columns were added: `season_best_streak`, `active_streak` (older files lack them; readers must ignore `streak` for those two tabs in pre-fix files and use `season_stats.best_streak` or the 9/22 full walk — `docs/audit/2026-09-corrections-index.md` C-01).
- **Fail-closed parsing.** `LeaderboardEnvelopeError` (error-only body, non-empty `errors`, missing `ranks`), `LeaderboardRowError` (no `userId`, or a season-best/all-time row without its `streak` field — no substitution); `validate_profile_envelope()` → `ProfileEnvelope` (required fields, nested `roundPredictions`/slot keys, raw counts, `unresolved_round_predictions` + reasons: missing unit/player, slot number ∉ {1,2}, pending result, settled result without atBats/hits) so an error/partial body can never parse as a user with zero picks. `SeasonStats` gained `user_id` (usernames are NOT unique: 72,600 ids ↔ 67,548 names on 9/22) and `skipped_unknown_round_predictions`; season-stats parquet gained `user_id`. `fetch_login_session(post=…)` accepts an injectable transport so a caller can archive/classify the raw login response before parsing.
- **One-pass season-board grab** — `scripts/final_leaderboard_grab.py` (owner-authorized end-of-season capture; runbook `docs/audit/2026-09-22-final-grab-runbook.md`): plan.json + status.json written BEFORE any network; intent-before-send request ledger; hard budget (or none — owner 9/22: "no request ceiling"); status observed BEFORE the body (streaming) so a 403/429 with an unreadable body still trips the global kill-switch; every raw body archived gzipped with secrets scrubbed (token, cookies, uid; JSON keys and `\uXXXX` escapes included) and both wire/archived digests recorded; validated terminal-page rules (short page needs `nextPage=false`; contradiction/unknown/ceiling/repeated-page/no-progress are truncation with nonzero exit); population coverage reported separately from walk exhaustion (`census` only with valid, constant participant count + server timestamp on every page); cohort A = top of the final board, B = frozen 2026-05-01 early cohort (`docs/audit/2026-09-22-early-cohort-2026-05-01.json`, 310 ids) fetched by id; id-keyed `user_picks/<user_id>.parquet` + `identity.json`; operator Ctrl-C = exit 130 persisted as `cancelled_by_operator`. Output lives under `data/leaderboard/final_grab_<YYYYMMDD>/` and never touches the daily corpus. **First run 2026-09-22:** 551 requests all 200; season board = 72,600 listed users (listing floor: season best ≥ 4; header count 121,625), 300 profiles, 58,604 pick rows 3/25→9/21; restic archive `982eaea28b02`. A supplemental run is planned for 2026-09-28.
- **Round boards carry picks.** The per-round leaderboard (`LEADERBOARD_ROUND_URL_TEMPLATE`) rows include `predictions` (that round's `roundPredictions` with unitId/playerId/result/hits/atBats) for every listed participant (~70k/round, paginated) — the basis of the planned round-board pick campaign (`docs/superpowers/specs/2026-09-22-round-board-campaign-design.md`, not yet built).
...
## Key Learnings
- **(2026-09-22) The season-best leaderboard lists only users with best ≥ 4 (72,600 of a 121,625 header count); its empty terminal page reports a different participant count than every full page; usernames collide (~5k duplicates among 72.6k ids) — key everything by `userId`.** Per-round boards embed each listed participant's picks, so field pick data comes from round walks, not profile fetches.
- **(2026-09-22) The backtest's 86% and the live 68–72% are not comparable:** the historical path compounds over realized PA rows and ranks afterward; live serves estimated-PA on a projected slate. Treat the gap as an information-set mismatch until the W1.2 bridge says otherwise (`docs/superpowers/plans/2026-09-14-season-wrap-plan.md`).
