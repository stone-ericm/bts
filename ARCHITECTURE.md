...
End-of-day health checks dispatched by `bts.health.runner.run_all_checks()`. Each check module returns 0+ `Alert` objects (level: INFO/WARN/CRITICAL); CRITICAL alerts DM Bluesky via `bts.dm`. 21 sources as of 2026-07-02:
...
**Scheduling:** twice daily via systemd timer (`bts-leaderboard.service` + `.timer`) at 10:00 ET / 01:00 ET (the 01:00 run captures settled outcomes; per-user profiles are settled-only, so "morning intentions" are NOT visible there — see the endpoint notes below). Deep scrape budget ~15 min at the jittered gap. Plus a `*/30` cron `bts leaderboard capture-static` (public files, no auth).
