...
14. **Single-seed benchmarking is dangerous (2026-04-14/15)**: LightGBM's hardcoded `random_state=42` across all training paths produced measurably different results from other seeds on the *same* code and data. Initial seeds 43-46 suggested seed=42 was a ~100σ extreme; the 16-seed audit (random + magnitude stratification) corrected this: seed=42 is +1.29σ on MDP P(57), and the real outlier is seed=6992150 at +2.70σ. **True MDP P(57) is 3.50% ± 2.11pp across 16 seeds, not the claimed 8.91%**. Every shipped feature and rejected experiment was validated against a lucky-ish seed, and single-seed deltas were dominated by noise: observed cross-seed std on a no-op feature is comparable to the supposed "wins" of individual features. Fixed by introducing `BTS_LGBM_RANDOM_STATE` env var and running multi-seed audits to re-validate every decision. **Lesson: for stochastic training pipelines, always report mean ± std across ≥5 seeds; single-seed deltas are worse than useless because they actively mislead.** Open thread: Option 7 architectural change (pooled-seed quality bins feed MDP + two-metric reporting of avg P@1 + mean MDP P(57)) pending Phase 1 profile re-run.
