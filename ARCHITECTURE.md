...
| `contest_state` | 1 | contest-account streak (the real MLB BTS streak driving live picks) missing/invalid/**stale** when expected. **Level-aware staleness** (gap = `latest_resolved_pick − source_date`): gap > `EXPECTED_OVERNIGHT_LAG_DAYS` (=1) → CRITICAL "STALE" (the frozen-screenshot-for-a-week incident class); gap == 1 → INFO **before noon ET, WARN at/after noon ET** (added 2026-06-12: a gap==1 that *persists past noon* is no longer "expected overnight" — it means fetches succeed but the contest never advanced, e.g. a delivered pick was never entered in the MLB app; the fetch-failure DM can't fire because nothing fails. WARN points at the entered-in-app question. Still never CRITICAL at gap==1 — the pick path is already conservative); no `source_date` → CRITICAL (unverifiable). The pick path stays strict (any gap ≥ 1 → freeze + disable doubles). Also WARNs on a legacy/expired manual override file. (check added #138; stale + legacy/expired WARN added #142; level-aware overnight-lag INFO added to stop nightly false-CRITICALs; noon-ET escalation added 2026-06-12.) |
...
An exploratory campaign testing per-pitch Statcast swing data (miss distance, swing timing, mechanics) as candidate features. No swing feature is in production `FEATURE_COLS`. Spec + plans in `docs/superpowers/specs|plans/2026-06-12-statcast-swing-*`. Modules: `bts.features.swing` (bronze `data/processed/swing_{season}.parquet` → denominator-preserving daily aggregates → `shift(1)` rolling, leak-free), `bts.validate.slate_rank` (paired daily NDCG@10 + season-stratified block bootstrap), `bts.experiment.swing_screen` (declarative arm registry + train-once/score runner). Stage 0 (bronze + QA) complete; Stage 1 screen in progress. **Key methodology lesson: the controls gate (sentinels + null arms) caught two distinct false-conclusion classes before any result was trusted** — coverage starvation in training, and negative controls that weren't clean nulls (availability flags carry roster signal). Detail in the spec; do NOT promote any swing feature without the full screen→confirm→FDR pipeline.
