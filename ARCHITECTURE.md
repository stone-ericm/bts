...
14. **Single-seed benchmarking is dangerous (2026-04-14)**: LightGBM's hardcoded `random_state=42` across all training paths produced measurably different results from seeds 43-46 on the *same* code and data. Seed=42 gave MDP P(57) = 6.22%, while seeds 43-46 clustered at ~2.55%. Every shipped feature, every rejected experiment, every policy parameter — all validated against a single lucky seed that turns out to be a ~100σ statistical outlier. **True expected MDP P(57) is closer to 3.3%, not the claimed 8.91%**. Fixed by introducing `BTS_LGBM_RANDOM_STATE` env var and running multi-seed audits to re-validate every decision. **Lesson: for stochastic training pipelines, always report mean ± std across ≥5 seeds; single-seed deltas are worse than useless because they actively mislead.**
